    ...
    return (
        <>
            {api.data?.map((anime, index) => {
                return (
                    <div className="w-min flex justify-center transition ease-in-out hover:scale-105 hover:shadow-lg hover:shadow-white duration-300 rounded-md" key={index}>
                        <Link href={`/anime/${anime.mal_id}`} className="w-60 max-sm:w-48 cursor-pointer flex items-center flex-col">
